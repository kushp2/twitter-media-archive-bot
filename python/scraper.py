...
def scrape_tweets (scraped_account):
    ...
    try:
        ...
        # Wait for the password input and enter the password and wait for page to load
        password = WebDriverWait(driver, 60).until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'input[name="password"]')))
        password.send_keys(password_str)
        password.send_keys(Keys.RETURN)
        time.sleep(25)

        # Open the Twitter profile page and wait for page to load
        driver.get(f"https://x.com/{scraped_account}")
        time.sleep(25)
    ...
    while True:  # Infinite loop for continuous scrolling
        try:
            ...
            for tweet in tweets:
                ...
                # Joins the image links to one object
                images_links = ', '.join(tweet_images) if tweet_images else "No Images"

                # If the tweet is not in the collected tweets
                if (tweet_text, tweet_date, images_links) not in tweets_collected:
                    tweets_collected.add((tweet_text, tweet_date, images_links))
